const itemLink  = require('./item'),
    ...
    app         = {
        ...
        createCharFeed: (data) => {
            let feed = utils.createRssFeed(),
                allP = [],
                p;

            feed.options.title = `${utils.capitalize(app.options.character)} on ${utils.capitalize(app.options.realm)}`;
        ...
        createGuildFeed: (data) => {
            ...
            feed.options.title = `${utils.capitalize(app.options.guild)} on ${utils.capitalize(app.options.realm)}`;
            feed.options.link = 'http://' + app.options.host + app.options.path;
            ...
            case ('ACHIEVEMENT'):
                return new Promise(
                    (resolve) => {
                        ...
                        rss.image = utils.createRssImage(item.achievement.icon);
                        resolve(rss);
                    });
            ...
            case ('LOOT'):
                return blizzard.wow.item({origin: app.options.region, id: item.itemId})
                    .then(response => {
                        ...
                        rss.image = utils.createRssImage(response.data.icon);
                        return rss;
                    });
